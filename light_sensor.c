...
#define MOTE_ROLE TYPE_LIGHT_SENSOR
...
PROCESS_THREAD(main_process, ev, data) {
    ...
    while(1) {
        ...
        etimer_set(&command_timer, CLOCK_SECOND * 30);

        PROCESS_YIELD_UNTIL(etimer_expired(&command_timer));
        
        int light_level = random_rand() % 129;

        LOG_INFO("Send DATA_PACKET to server\n"); 
        char payload[64];
        sprintf(payload, "%d|%d", my_node.role, light_level); 

        send_data_packet(&my_node, payload);
